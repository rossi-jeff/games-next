...
import SpiderDirection from "@/components/spider-directions";
...
export default function SpiderPage() {
  ...
  const quit = () => {
    ...
    let Stock: Card[] = [];
    for (const id in Tableau) Tableau[id] = [];
    for (const id in Aces) Aces[id] = [];
    setAces(Aces);
    setStock(Stock);
    setTableau(Tableau);
  ...
  const updateGame = async (Status: GameStatus) => {
    if (!spider.id) return;
    try {
      const result = await fetch(`${apiUrl}/api/spider/${spider.id}`, {
        method: "PATCH",
        body: JSON.stringify({
          Moves: moves.current,
          Elapsed: elapsed,
  ...
  const countAces = () => {
    let count = 0;
    for (const id in aces) count += aces[id].length;
    return count;
  };
    ...
    <div id="spider-game" className="m-2">
      ...
      <div className="flex flex-wrap justify-between">
        <div>
          ...
          {spider.Status == "Playing" && <button onClick={quit}>Quit</button>}
        </div>
        <div>
          <strong className="mr-2">Status</strong>
          {spider.Status ? spider.Status : "N/A"}
        </div>
        <div>
      ...
      {spider.Status != "Playing" && <SpiderDirection />}
    </div>
  );
}
